...
# ---------------------------------------------------------
@st.cache_data
def get_bands_from_image(_image: ee.Image):
...
# ---------------------------------------------------------
ASSETS = {
    "North Arm (Pre-calculated Indices)": 'projects/ut-gee-ugs-bsf-dev/assets/GSLDH_NABRB_Unclassified_Landsat_Col',
    "South Arm (Pre-calculated Indices)": 'projects/ut-gee-ugs-bsf-dev/assets/GSLDH_SA_Unclassified_Landsat_Col',
...
PRIMARY_RGB_MATCH = {
    "North Arm Temporal Anomalies": "North Arm (Pre-calculated Indices)",
    "South Arm Temporal Anomalies": "South Arm (Pre-calculated Indices)"
}
...
def calculate_exposure_duration(binary_water_collection, water_band='water_mask'):
    ...
    Returns:
        ee.ImageCollection: A collection where pixel values represent days since last inundation.
    """
    ...
    # We pass a dictionary containing the 'last_seen' timestamp image and a list to collect results.
    # We initialize the timestamp image at 0 (meaning "never seen water").
    ...
    def _track_exposure(image, state):
        ...
        # ---------------------------------------------------------------------
        # THE WIZARD TRICK: Vectorized If/Else
        # Update the 'last_water_timestamp'. 
        # If water == 1, update to current_date. Else, keep previous_timestamp.
        # ---------------------------------------------------------------------
        updated_timestamp_img = previous_timestamp_img.where(water_mask.eq(1), current_date_img)
        
        # Calculate the difference in milliseconds, then convert to days.
        # If water == 1, this evaluates to (current - current) = 0 days.
        # If water == 0, this evaluates to (current - previous) = N days.
        ...
        # Mask out pixels that have NEVER been water in the sequence 
        # (where updated_timestamp is still our initialized 0)
        ...
        # Return the updated dictionary to the next step of the iteration
...
# def calc_pixel_stability():
#     """PASTE YOUR CODE HERE: Return a temporally reduced single ee.Image"""
...
product_type = st.sidebar.radio(
    ...
    ["GEE Asset Collections", "On-the-Fly Products"]
)
...
if product_type == "GEE Asset Collections":
    ...
    available_bands = get_bands_from_image(selected_image_ee)
    ...
    if product_name in PRIMARY_RGB_MATCH:
        ...
        # Look up the exact date in the primary collection to ensure alignment
        ...
        # If it's already a primary collection, the RGB is in the selected image
    ...
    if product_name == "NABRB Days Since Last Water (NDWI)":
        ...
        NA_water_timing_mndwi_col = LandsatCollection(collection=calculate_exposure_duration(NA_mndwi.collection, water_band='mndwi'))
        NA_water_timing_ndwi_col = LandsatCollection(collection=calculate_exposure_duration(NA_ndwi.collection, water_band='ndwi'))
        # SA_water_timing_mndwi_col = LandsatCollection(collection=calculate_exposure_duration(SA_mndwi.collection, water_band='mndwi'))
        # SA_water_timing_ndwi_col = LandsatCollection(collection=calculate_exposure_duration(SA_ndwi.collection, water_band='ndwi'))
        ...
        # If your on-the-fly collection code retains RGB bands, map them here:
...
# Band Selection
viz_band = st.sidebar.selectbox("Select Band to Visualize:", available_bands)

# Palette, Min, Max
col1, col2 = st.sidebar.columns(2)
viz_min = col1.number_input("Min", value=0.0)
viz_max = col2.number_input("Max", value=1.0)

# palette_choice = st.sidebar.selectbox(
#     "Color Palette:", 
#     ["viridis", "cividis", "Reds", "Blues", "inferno", "magma"] 
# )

palette_choice = st.sidebar.selectbox(
    "Color Palette:", 
    ["inferno", "magma", "viridis", "cividis", "reds", "blues", "rdbu"],
    0
)
...
if gee_ready and selected_image_ee:
    st.subheader(f"Viewing: {product_name}")
    
    Map = geemap.Map(center=[41.15, -112.5], zoom=9)
    
    # ADD RGB BASE LAYER (Only if it's a collection and RGB was successfully found)
